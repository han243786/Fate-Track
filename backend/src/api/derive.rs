...
pub fn derive(request: &Request) -> Result<Response, crate::error::AppError> {
    ...
    let body = if dt == "summary" {
        format!(
            r#"{{"status":"restricted","capability":"data-derivation","type":"summary","total_cases":{},"privacy":{{"threshold":{},"note":"No individual data. Aggregates suppressed below {} cases."}}}}"#,
            total, MIN_THRESHOLD, MIN_THRESHOLD
        )
...
fn to_json(ty: &str, total: u32, map: &std::collections::BTreeMap<String, u32>) -> String {
    let entries: Vec<String> = map
        ...
        .collect();
    format!(
        r#"{{"status":"restricted","capability":"data-derivation","type":"{}","total_cases":{},"threshold":{},"data":{{{}}}}}"#,
        ty,
        total,
        MIN_THRESHOLD,
        entries.join(",")
