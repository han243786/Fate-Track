...
pub fn assess_strength(chart: &ChartResult) -> StrengthAssessment {
    ...
    let branches = [
        ...
        chart
            ...
            .as_ref()
            .map(|h| &h.branch)
            .unwrap_or(&chart.chart.day.branch),
    ];
    let dedi = branches
        .iter()
        .filter(|b| {
    ...
    let stems = [
        ...
        chart
            ...
            .as_ref()
            .map(|h| &h.stem)
            .unwrap_or(&chart.chart.day.stem),
    ];
    let deshi = stems
        .iter()
        .filter(|s| {
    ...
    let score = if deling { 4 } else { 0 } + dedi.min(3) + deshi.min(3);
...
pub fn classify_pattern(chart: &ChartResult, _strength: &StrengthAssessment) -> PatternInfo {
    ...
    if controls(month_el, day_el) {
        ...
        if yang == m_yang {
            return PatternInfo {
                pattern_name: "七杀格".into(),
                pattern_type: "正格",
                confidence: "确定",
            };
        } else {
            return PatternInfo {
                pattern_name: "正官格".into(),
                pattern_type: "正格",
                confidence: "确定",
            };
        }
    ...
    if day_el == month_el {
        ...
        if yang {
            return PatternInfo {
                pattern_name: "建禄格".into(),
                pattern_type: "正格",
                confidence: "确定",
            };
        } else {
            return PatternInfo {
                pattern_name: "月刃格".into(),
                pattern_type: "正格",
                confidence: "确定",
            };
        }
