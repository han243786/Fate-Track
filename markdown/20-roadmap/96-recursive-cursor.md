...
| `state` | `closing` |
| `allowed_scope` | M0-M28 全部关闭。Final matrix: 10 supported, 7 restricted, 0 target, 0 planned。边界锁定。 |
...
| `required_governance_sync` | Complete — M28 milestone + module tree + engineering tree + roadmap index |
| `validation_commands` | `cargo test --lib 86 passed; npm run check 10 passed; cargo check -p minggui-desktop` |
| `last_green_gate` | Governance OK, 86 Rust + 10 frontend, desktop shell compiles |
| `last_closeout` | `markdown/20-roadmap/97-loop-closeout-log.md#loop-089` |
| `next_resume_instruction` | 无。M0-M28 全部完成。边界锁定。回归保护：`cargo test` + `npm run check`。
